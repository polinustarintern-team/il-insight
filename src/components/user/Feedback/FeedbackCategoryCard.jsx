...
const FeedbackCategoryCard = ({ title, description, isDark, path, onClick, disabled = false }) => {
    ...
    return (
        <div className={`rounded-3xl p-8 flex flex-col items-center text-center border h-full ${isDark ? 'bg-[#110e2d] border-[#110e2d]' : 'bg-white border-gray-200'} ${disabled ? 'opacity-60' : ''}`}>
            ...
            <button
                onClick={handleClick}
                disabled={disabled}
                className={`w-full py-3 rounded-full font-bold text-sm transition-colors ${disabled
            ...
            >
                {disabled ? 'Not Available' : 'Start Feedback'}
            </button>
        </div>
    );
};
